...
def invest_decumulate_NN_strategy(NN_pyt, params):
    ...
    # params["qsum_vector"]: contains cumulative sum of q(t_N+) using withdrawal strategy given by NN_withdraw
    #              qsum_vector.shape = (N_d, 1) since it contains q_n for each Rb_n
    ...
    # withdrawal history and cumulative sum
    params["q_matrix"] = np.zeros([N_d, N_rb+1]) #   W contains PATHS, so W.shape = (N_d, N_rb+1)
    params["qsum_T_vector"] = np.zeros([N_d, 1]) #   W contains PATHS, so W.shape = (N_d, N_rb+1)
