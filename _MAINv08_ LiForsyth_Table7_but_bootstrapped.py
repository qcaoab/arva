...
run_on_my_computer = True   #True if running in my local desktop, False if running remotely
...
code_title_prefix = "z_MAINv08_LiForsyth_boot_"    #used for saving output
...
params["W0"] = 0.     # Initial wealth W0
params["q"] =  10. * np.ones(params["N_rb"])  # Cash injection schedule (a priori specified)
...
params["N_d_train"] = 500000 #int(2.56* (10**6)) #Nr of TRAINING data return sample paths to bootstrap
...
params["test_TrueFalse"] = True #TRUE if training AND testing, FALSE if *just training*

if params["test_TrueFalse"] is True:
    params["N_d_test"] = 100000  # (only used when test_TrueFalse == True) Nr of TESTING data sample paths to bootstrap
...
params["asset_basket_id"] = "basic_ForsythLi"    #Pre-defined basket of underlying candidate assets - see fun_Data_assets_basket.py
...
# -----------------------------------------------------------------------------------------------
params["obj_fun"] = "one_sided_quadratic_target_error"
...
tracing_parameters_to_run = [805.]  #Must be LIST
...
itbound = 64000 #64000    #Mean-CVAR: use at least itbound = 50k
...
output_parameters["output_Rolling_Historical_Data_test"] = True  #If true, test NN strategy and benchmark strategy on actual single
                                    # historical data path, starting in each month and investing for the duration
output_parameters["fixed_yyyymm_list"] = [198001, 198501, 199001] #Used for historical rolling test; LIST of yyyymm_start months of particular interest
output_parameters["output_Rolling_Historical_only_for_fixed"] = True  #if True, outputs rolling historical ONLY for output_parameters["fixed_yyyymm_list"]

# NN detail
output_parameters["output_TrainingData_NNweights_test"] = True   #if true, outputs the training paths + features + NN weights required to
...
output_parameters["output_Pctiles_Plots_W_max"] = 1000. #Maximum y-axis value for WEALTH percentile plots
...
output_parameters["heatmap_y_bin_max"] = 1000.0  # maximum for the y-axis grid of heatmap
...
if params["data_source_Train"] == "bootstrap":  # TRAINING data bootstrap
    ...
    params = fun_Data__bootstrap_wrapper.wrap_run_bootstrap(
        ...
        data_bootstrap_yyyymm_end = 200912,         # end month to use subset of data for bootstrapping, CHECK DATA!
        data_bootstrap_exp_block_size = 6,          # Expected block size in terms of frequency of market returns data
    ...
    if params["use_trading_signals_TrueFalse"]  is True:
        ...
        params = fun_Data__MCsim_wrapper.wrap_run_MCsim(
                    train_test_Flag = "train",  # "train" or "test"
                    params = params,  # params dictionary as in main code
                    model_ID_set_identifier = "ForsythLi_2019_basic" # (see code)identifier for the collection of models AND correlations to use
                   )
        # ASSET return data:
...
if params["test_TrueFalse"] is True:
    ...
    if params["data_source_Test"] == "bootstrap": #TESTING data bootstrap
        ...
        params = fun_Data__bootstrap_wrapper.wrap_run_bootstrap(
            ...
            params = params,                            # params dictionary as in main code
            data_bootstrap_yyyymm_start = 201001,       # start month to use subset of data for bootstrapping, CHECK DATA!
